...
# 获取所有的训练图片
TrainX = np.load('foot_data_crop_center_train\\TrainX.npy')
TrainY = np.load('foot_data_crop_center_train\\TrainY.npy')
TestX = np.load('foot_data_crop_center_train\\TestX.npy')
TestY = np.load('foot_data_crop_center_train\\TestY.npy')
...
if __name__ == '__main__':
    ...
    for i in range(50001):
        ...
        if i % 100 == 0 and i != 0:
            summary, test_acc = sess2.run([merged, accuracy], {h0: TestX, y_: TestY})
            print(str(i) + ": test --->" + "accuracy: " + str(test_acc))
            test_writer.add_summary(summary, i)
        if i % 5000 == 0 and i != 0:
            foot_saver.save(sess2, 'crop_center_checkpoint/%05d.ckpt' % i)
