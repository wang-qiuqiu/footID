...
def predict_one_img_class(img_path):
    ...
    img = np.array(img_list)
    with tf.Session() as sess:
        foot_saver.restore(sess, 'checkpoint\\20000.ckpt')
...
def compute_distance_in_class(target_img_path):
    ...
    with tf.Session() as sess:
        foot_saver.restore(sess, 'checkpoint\\40000.ckpt')
        h_test_predict = sess.run(tf.nn.softmax(y), {h0: img})
    ...
    # 加载源文件
    source = np.load('std_similarity\\std_similarity.npy')
    target = ''
...
if __name__ == '__main__':
    ...
    target_img_path = 'C:\\Users\\Neo\\Desktop\\1.jpg'
    compute_distance_in_class(target_img_path)
